...
generic(selectnum : integer:=  3; --number of selected TOBs
   ...
   Port(threshold     : in  std_logic_vector(      9 downto 0);
        ...
        clusterTOBarr : in  clusterArray (inchn-1     downto 0); --input TOBs
        selTOBarr     : out clusterArray (selectnum-1 downto 0); --selected TOBs
...
type maxarr    is array (integer range <>) of clusterArray     (2*inchn-2 downto 0); --array of possible outputs for each of the selected TOBs
...
gen_cmp_res1: for i in 0 to inchn-1 generate --flag every TOB greater than the threshold
 gen_cmp_res2a: if do_presel = 1 generate
  cmp_res(i) <= '1' when clusterTOBarr(i).Et>threshold else '0';
 end generate;
end generate;
...
gen1: for i in 0 to inchn-1 generate --generate a selection matrix
 ...
 genmaxi: for k in 0 to selectnum-2 generate
  ...
  maxa(k)(i)<=clusterTOBarr(i) when selection(k)(i)=fullscale else (dummyTOB); -- assign the the first k TOBs to the according temporary output
 ...
 maxa(selectnum-1)(i)<=clusterTOBarr(i) when selection(selectnum-1)(i)=fullscale else (dummyTOB); -- assign the last selected TOB to the according temporary output
...
genTmpOut: for k in 0 to selectnum-1 generate  -- connect every possible temporary output for the k selected TOBs with an OR-tree
 genx: for i in 0 to inchn-2 generate
  maxa(k)(inchn+i).Et  <= maxa(k)(2*i).Et  or maxa(k)(2*i+1).Et; 
